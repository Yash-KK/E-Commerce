    ...
    if request.user.is_authenticated:
        if request.method == 'POST':
            ...
            for item in request.POST:
                ...
                try:
                    ...
                    print("Nahi HUa")       
        cart_item = CartItems.objects.filter(user=current_user, product=product).exists()
        if cart_item:
            cart_item = CartItems.objects.filter(user=current_user, product=product)
        ...
        # CONTINUE FROM HERE-----------------------> IF THE PRODUCT VARIATIONS EXIST IN EXISTING VARIATIONS ,,,,,,
            ...
            if product_ke_variations in exist_var:
                ...
                item = CartItems.objects.create(product=product, user=current_user, quantity=1)
...
def decrement_item(request, id,cart_item_id):
    ...
    try:
        if request.user.is_authenticated:
            cart_item = CartItems.objects.get(user=request.user,product=product, id=cart_item_id)
        else:
            cart = Cart.objects.get(cart_id=_cart_id(request))
